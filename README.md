...
The plugin exposes a single accessory with a TV-style input selector, plus mute and volume under the same device. Volume is available both as a `Lightbulb` dimmer slider tile (a draggable control in the Home app) and via a `TelevisionSpeaker` linked to the TV (iPhone/iPad hardware volume buttons and the Control Center Remote); the two stay in sync.
